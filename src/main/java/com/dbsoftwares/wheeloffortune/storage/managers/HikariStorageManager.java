...
public abstract class HikariStorageManager extends AbstractStorageManager {
    ...
    public HikariStorageManager(final StorageType type, final ISection section) {
        ...
        config.setPoolName("WheelOfFortune");
        config.setLeakDetectionThreshold(10000);
        config.setConnectionTestQuery("/* WheelOfFortune ping */ SELECT 1;");
    ...
    public boolean isOptedIn(UUID uuid) {
        ...
        try (Connection connection = getConnection();
             PreparedStatement pstmt = connection.prepareStatement("SELECT * FROM wof_data WHERE uuid = ? LIMIT 1;")) {
    ...
    public void optIn(UUID uuid) {
        try (Connection connection = getConnection();
             PreparedStatement pstmt = connection.prepareStatement("DELETE FROM wof_data WHERE uuid = ?;")) {
            pstmt.setString(1, uuid.toString());
    ...
    public void optOut(UUID uuid) {
        try (Connection connection = getConnection();
             PreparedStatement pstmt = connection.prepareStatement("INSERT INTO wof_data VALUES (?);")) {
            pstmt.setString(1, uuid.toString());
